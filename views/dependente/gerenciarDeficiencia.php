...
<html >
    <body>  
        <?php
             $formattedData = [];
            foreach ($deficiencias_dependentes as $deficiencia_dependente) {
                $formattedData[] = [
                    'id' => $deficiencia_dependente->id,
                    'id_dependente' => $deficiencia_dependente->id_dependente,
                    'nome' => $deficiencia_dependente->nome,
                    'id_deficiencia' => $deficiencia_dependente->id_deficiencia,
                    'nome_deficiencia' => $deficiencia_dependente->descricao,
                    'data_cadastro' => date('d/m/Y \à\s H:i:s', strtotime($deficiencia_dependente->cadastro_deficiencia_data)),
                    'editar' => "<button class='editar btn btn-link' data-nome='$deficiencia_dependente->nome' data-descricao='$deficiencia_dependente->descricao' data-id='$deficiencia_dependente->id' data-id_dependente='$deficiencia_dependente->id_dependente' data-id_deficiencia='$deficiencia_dependente->id_deficiencia'><i class='bi bi-pencil'></i></button>",
                    'excluir' => "<button class='excluir btn btn-link' data-nome='$deficiencia_dependente->nome' data-descricao='$deficiencia_dependente->descricao' data-id='$deficiencia_dependente->id' data-id_dependente='$deficiencia_dependente->id_dependente' data-id_deficiencia='$deficiencia_dependente->id_deficiencia'><i class='bi bi-trash'></i></button>"
                ];
            }
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item active" aria-current="page"><span class="text-breadcrumb-blue texto-breadcrumb">Gerenciar Tipo de Deficiência de Dependente</span></li>
            </ol>
        </nav>

        <h1>Gerenciar Tipo de Deficiência de Dependente</h1>

        <table id="deficienciaTable" class="display">
            <thead>
                <tr>
                    <th>Id</th>
                    <th>Dependente</th>
        ...
        <div class="modal fade" id="confirmationModal" tabindex="-1" aria-labelledby="confirmationModalLabel" aria-hidden="true">
            <div class="modal-dialog">
                <form id="updateForm" action="<?= url_for('dependente/deficiencia'); ?>" method="post">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="confirmationModalLabel">Alteração de Tipo de Deficiência</h5>
                        ...
                        <input type="hidden" id="modalTipoAcao" name="modalTipoAcao"> <!-- tipo da ação realizada na modal -->

                        <input type="hidden" id="modalIdDependente" name="modalIdDependente">

                        <div class="form-group">
                            <label for="modalNome">Nome do Dependente</label>
                            <input type="text" class="form-control" id="modalNome" name="modalNome" maxlength="50" disabled>
                        </div>

                        <div class="form-group edit-form">
                            <label for="modalDeficiencia">Tipo de Deficiência</label>
                            <select class="form-control select-modal" id="modalDeficiencia" name="modalDeficiencia">
                                <?php
                        ...
                        <p  id="texto-confirm" style="text-align: left;display:block;margin-top: 20px; margin-bottom: 10px;"></p>
        ...
        <script>    
            var jsonData = <?php echo json_encode($formattedData); ?>;</script>
        </script>
...
<!-- Js da página -->
<script src="assets/js/dependente/gerenciar_deficiencia.js"></script>
